...
//    CGFloat btnH = 30.0f;
    ...
    CGFloat btnW = (self.width - 100.0f)/3;
    CGFloat btnH = 30.0f;
    ...
    for(int i=0;i<9;i++)
    {
      //  ClassifyTagsDTO *tagDTO = [self.dataArray objectAtIndex:i];
        HobbyItemBtn *btn = [[HobbyItemBtn alloc]initWithFrame:CGRectMake(spaceX*4+self.width*(i/9)+(spaceX+btnW)*(i%3),120.0f+spaceY+ (spaceY+btnH)*((i/3)%3), btnW, btnH)];
